...
exports.seed = function(knex, Promise) {
  return Promise.join(
    ...
    knex('reviews').insert({res_id: knex('restaurants').select('id').where('name', 'Los Tacos'),
      rev_name: 'Jane Smith',
      rev_date: '2016-02-27',
      rating: 5,
      review: 'I loved this place! Best mexican food ever! Get the chimichangas.'}),
    knex('reviews').insert({res_id: knex('restaurants').select('id').where('name', 'Burger Bar'),
      rev_name: 'Joe Schmoe',
      rev_date: '2016-02-26',
      rating: 5,
      review: 'The burgers at Burger Bar are the best! For the best burger get the bison burger.'}),
    knex('reviews').insert({res_id: knex('restaurants').select('id').where('name', 'Pasta Freddys'),
    ...
    knex('reviews').insert({res_id: knex('restaurants').select('id').where('name', 'Bangkok Grill'),
    ...
    knex('reviews').insert({res_id: knex('restaurants').select('id').where('name', 'Pho Mazing'),
      rev_name: 'Max Jackson',
      rev_date: '2016-01-01',
      rating: 2,
      review: 'Came here for a hangover cure after New Year\'s Eve, got nothing but some grody soup.'}),
    knex('reviews').insert({res_id: knex('restaurants').select('id').where('name', 'Fiestaritos'),
